...
import { extractStays } from "@/lib/geo/review";
...
export default function Review() {
  ...
  const load = useCallback(() => {
    if (!trip) return;
    getTripCurrency(trip).then(setCurrency);
    // turn any tracked points into stays before showing the trail
    extractStays(trip).then(() => getTripReview(trip).then(setStays));
  }, [trip]);
  useFocusEffect(useCallback(() => load(), [load]));

  const gaps = stays.filter(
    (s) => !s.expense && s.review_status === "unreviewed",
  ).length;
  const logged = stays.filter(
    (s) => s.expense || s.review_status !== "unreviewed",
  ).length;
      ...
      <ScrollView>
        <View className="px-5 pt-2 pb-16">
          ...
          <Eyebrow className="mt-1">
            {stays.length} stops · {logged} logged ·{" "}
            {gaps ? `${gaps} to recall` : "all recalled"}
          </Eyebrow>
          <Rule className="mt-3" />
          ...
          {stays.map((s) => {
            ...
            return (
              <View key={s.id} className="py-3 border-t border-hair">
                ...
                {isGap ? (
                  <View className="mt-2">
                    <Text className="text-amber-ink font-bold mb-2">
                      You stopped here. Spend anything?
                    </Text>
                      ...
                      <Pressable
                        onPress={() =>
                          router.push({
                            ...
                            params: { tripId: trip, currency, stayId: s.id },
                          })
                        }
                        className="bg-amber rounded-md px-3 py-2"
                      >
                        <Text className="text-on-amber font-bold">
                          + Add expense
                        </Text>
                      </Pressable>
                      <Pressable
          ...
          {stays.length === 0 ? (
            <Text className="text-carbon mt-4">
              No stops yet. Turn on tracking for this trip and your day will
              fill in here.
            </Text>
          ) : null}
        </View>

...
export async function seedIfEmpty(): Promise<void> {
  ...
  const daily = async (minor: number, cat: string, note: string, ts: string) =>
    putRow(
      ...
      "expenses",
      {
        ...
        estimated_home_amount: null,
        actual_home_amount: null,
        receipt_id: null,
        stay_id: null,
        created_at: now,
        updated_at: now,
  ...
  // A shared travel expense: you fronted it, split equally with Yana. The stored
  // split shares are in the expense currency (VND); settlement re-derives them in
  // home units from the participant set (see getSettlement).
  const travAt = async (
    minor: number,
    note: string,
    estPhp: number,
    ...
    const shares = splitEvenly(minor, 2);
    await putRow(
      db,
      "splits",
      { expense_id: eid, person_id: me, share_amount: shares[0] },
      S,
    );
    await putRow(
      db,
      "splits",
      { expense_id: eid, person_id: yana, share_amount: shares[1] },
      S,
    );
  };
